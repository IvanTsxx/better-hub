    ...
    <div>
      ...
      {loading && (
        <div className="flex flex-col items-center justify-center py-20 gap-3">
          ...
          <span className="text-[11px] font-mono text-muted-foreground/25">Loading run data...</span>
        </div>
      )}
      {error && (
        <div className="flex flex-col items-center justify-center py-20 gap-2">
          ...
          <p className="text-[12px] font-mono text-destructive/80">{error}</p>
        </div>
      )}
      {data && data.length === 2 && (
        <RunComparisonDiff runs={data as [ComparisonRun, ComparisonRun]} owner={owner} repo={repo} />
      )}
      {data && data.length > 2 && (
