...
function StatusCell({ job, maxDuration }: { job: ComparisonRun["jobs"][number] | undefined; maxDuration: number }) {
  if (!job) {
    return (
    ...
    <td className={cn(
      ...
      <div className="flex flex-col items-center gap-1">
        <div className="flex items-center gap-1.5 justify-center">
          <StatusIcon status={job.status} conclusion={job.conclusion ?? null} className="w-3 h-3" />
          {isSkipped ? (
            <span className="text-[11px] font-mono text-muted-foreground/25">skip</span>
          ) : isCancelled ? (
            <span className="text-[11px] font-mono text-muted-foreground/25">cancel</span>
          ) : (
            <span className="text-[11px] font-mono tabular-nums">{formatDurationFromSeconds(duration)}</span>
          )}
        </div>
        {/* Mini duration bar */}
        {!isSkipped && !isCancelled && maxDuration > 0 && (
          <div className="h-0.5 w-full max-w-[80px] bg-muted/15 overflow-hidden mx-auto">
            <div
              className={cn(
                "h-full",
                job.conclusion === "success" && "bg-success/30",
                job.conclusion === "failure" && "bg-destructive/30",
...
export function RunComparisonTimeline({ runs, owner, repo }: { runs: ComparisonRun[]; owner: string; repo: string }) {
  ...
  const maxJobDuration = Math.max(
    ...runs.flatMap((r) => r.jobs.map((j) => calculateDuration(j.started_at, j.completed_at))),
    1
  );

  // Compute total durations for trend
  const totalDurations = runs.map((r) => calculateDuration(r.run.run_started_at, r.run.updated_at));
  const minTotal = Math.min(...totalDurations);
  const maxTotal = Math.max(...totalDurations);

  // Count failures per run
  const failureCounts = runs.map((r) => r.jobs.filter((j) => j.conclusion === "failure").length);
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center gap-4 flex-wrap">
        <span className="text-[10px] font-mono uppercase tracking-wider text-muted-foreground/30">
          {runs.length} runs · {allJobNames.length} job{allJobNames.length !== 1 ? "s" : ""}
        </span>
        {minTotal !== maxTotal && (
          <div className="inline-flex items-center gap-2 px-3 py-1.5 text-[11px] font-mono border border-border/20 text-muted-foreground/40">
            Duration range: {formatDurationFromSeconds(minTotal)} — {formatDurationFromSeconds(maxTotal)}
          </div>
        )}
      </div>
        ...
        <table className="w-full">
          <thead>
            <tr className="border-b border-border/30">
              ...
              {runs.map((r) => (
                <th key={r.run.id} className="px-3 py-3 text-center font-normal min-w-[130px]">
                  <div className="flex flex-col items-center gap-1.5">
                    ...
                    >
                      <StatusIcon status={r.run.status ?? ""} conclusion={r.run.conclusion ?? null} className="w-3 h-3" />
                      <span className="text-[12px] font-mono font-medium tabular-nums">#{r.run.run_number}</span>
                    </Link>
                    ...
                    {r.run.head_branch && (
                      ...
                      >
                        ...
                        <span className="text-[10px] font-mono text-muted-foreground/30 truncate">{r.run.head_branch}</span>
                      </Link>
                    )}

                    {/* Time ago */}
                    <span className="text-[10px] font-mono text-muted-foreground/20">
                      <TimeAgo date={r.run.updated_at} />
                    </span>
                  </div>
                </th>
              ))}
            ...
            {allJobNames.map((jobName, idx) => (
              <tr key={jobName} className={cn(
                ...
                {runs.map((r) => (
                  <StatusCell
                    key={r.run.id}
                    job={r.jobs.find((j) => j.name === jobName)}
          ...
          <tfoot>
            <tr className="border-t border-border/30 bg-muted/[0.04]">
              ...
              {runs.map((r, i) => {
                ...
                const isFastest = total === minTotal && minTotal !== maxTotal;
                const isSlowest = total === maxTotal && minTotal !== maxTotal;
                return (
                  <td key={r.run.id} className="px-3 py-2.5 text-center">
                    <div className="flex flex-col items-center gap-0.5">
                      <span className={cn(
                        "text-[12px] font-mono font-medium tabular-nums",
                        isFastest && "text-success",
                        isSlowest && "text-destructive/70"
                      )}>
                        {formatDurationFromSeconds(total)}
                      </span>
                      {isFastest && <span className="text-[9px] font-mono text-success/50">fastest</span>}
                      {isSlowest && <span className="text-[9px] font-mono text-destructive/40">slowest</span>}
                    </div>
                  </td>
                );
              })}
