  ...
  return (
    <div>
      <div className="flex items-center gap-2 px-3 pt-2.5 pb-1">
        ...
        {!isDefault && (
          <button
            onClick={() => { onChange(options[0]?.value ?? "all"); setSearch(""); }}
      ...
      {searchable && options.length > 6 && (
        <div className="flex items-center gap-1.5 mx-3 mb-1 px-2 py-1 border border-border/40 bg-muted/10">
          ...
          />
          {search && (
            <button onClick={() => setSearch("")} className="text-muted-foreground/30 hover:text-muted-foreground cursor-pointer">
              <X className="w-2.5 h-2.5" />
            </button>
          )}
        </div>
      )}
      <div className={cn("overflow-y-auto", searchable && options.length > 6 ? "max-h-[160px]" : "max-h-[200px]")}>
        {filtered.map((opt) => (
          <button
            key={opt.value}
            ...
            className={cn(
              ...
              value === opt.value && "text-foreground font-medium bg-muted/20"
            )}
          >
            {opt.label}
          </button>
      ...
      <div className="flex items-center gap-1.5">
        ...
        {activeFilters.map((f) => {
          ...
          return (
            ...
            >
              <span className="text-blue-400/40">{f.label}:</span>
              <span className="truncate max-w-[80px]">{selected?.label ?? f.value}</span>
              <button
                onClick={(e) => { e.stopPropagation(); f.onChange(f.options[0]?.value ?? "all"); }}
          ...
          <div className="absolute top-full left-0 mt-1 z-20 min-w-[260px] border border-border bg-background shadow-lg">
            {filters.map((filter, fi) => (
              <div key={filter.label}>
                {fi > 0 && <div className="h-px bg-border/20" />}
                <FilterSection
                  label={filter.label}
                  value={filter.value}
...
function WorkflowFilterDropdown({
  ...
  return (
    <div className="relative">
      ...
      >
        <span className={isDefault ? "text-muted-foreground/60" : "text-blue-400/60"}>Workflow</span>
        <span className="truncate max-w-[140px]">{selected?.name ?? "All workflows"}</span>
        ...
        <>
          ...
          <div className="absolute top-full left-0 mt-1 z-20 min-w-[260px] border border-border bg-background shadow-lg">
            <div className="flex items-center gap-2 px-2.5 py-2 border-b border-border/60">
              ...
              />
              {search && (
                <button onClick={() => setSearch("")} className="text-muted-foreground/40 hover:text-muted-foreground cursor-pointer">
                  <X className="w-3 h-3" />
                </button>
              )}
            </div>
            <div className="max-h-[260px] overflow-y-auto">
              <button
                ...
                className={cn(
                  ...
                  !value && "text-foreground font-medium bg-muted/20"
                )}
              >
                All workflows
              </button>
              {filtered.map((wf) => (
                <button
                  ...
                  className={cn(
                    ...
                    value === wf.id && "text-foreground font-medium bg-muted/20"
                  )}
                >
                  {wf.name}
                </button>
...
export function ActionsList({
  ...
  const initialWorkflowId = useMemo(() => {
    ...
    const wf = workflows.find(
      (w) => w.path === `.github/workflows/${initialWorkflow}` || w.path === initialWorkflow
    );
    return wf?.id ?? null;
  ...
  const eventOptions = useMemo(() => {
    ...
    return [{ value: "all", label: "All events" }, ...events.map((e) => ({ value: e, label: e }))];
  }, [displayRuns]);

  const branchOptions = useMemo(() => {
    const branches = [...new Set(displayRuns.map((r) => r.head_branch).filter(Boolean) as string[])].sort();
    return [{ value: "all", label: "All branches" }, ...branches.map((b) => ({ value: b, label: b }))];
  }, [displayRuns]);

  const actorOptions = useMemo(() => {
    const actors = [...new Set(displayRuns.map((r) => r.actor?.login).filter(Boolean) as string[])].sort();
    ...
    return displayRuns.filter((run) => {
      if (statusFilter !== "all") {
        if (statusFilter === "in_progress") {
          if (run.status !== "in_progress" && run.status !== "queued" && run.status !== "waiting") return false;
        } else {
          if (run.conclusion !== statusFilter) return false;
        }
      }
      if (eventFilter !== "all" && run.event !== eventFilter) return false;
      if (branchFilter !== "all" && run.head_branch !== branchFilter) return false;
      if (actorFilter !== "all" && run.actor?.login !== actorFilter) return false;
      return true;
    });
  }, [displayRuns, statusFilter, eventFilter, branchFilter, actorFilter]);

  const hasActiveFilters = statusFilter !== "all" || eventFilter !== "all" || branchFilter !== "all" || actorFilter !== "all";

  const pushWorkflowUrl = useCallback((wf: Workflow | undefined) => {
    if (wf) {
      const file = wf.path.replace(/^\.github\/workflows\//, "");
      window.history.pushState(null, "", `/${owner}/${repo}/actions/workflows/${file}`);
    } else {
  ...
  return (
    <div>
        ...
        <>
          <div className="flex items-center gap-2 flex-wrap">
            ...
            <CombinedFilterDropdown
              filters={[
                { label: "Status", value: statusFilter, options: STATUS_OPTIONS.map((o) => ({ value: o.value, label: o.label })), onChange: (v) => setStatusFilter(v as StatusFilter) },
                { label: "Event", value: eventFilter, options: eventOptions, onChange: (v) => setEventFilter(v) },
                { label: "Branch", value: branchFilter, options: branchOptions, onChange: (v) => setBranchFilter(v), searchable: true },
                { label: "Actor", value: actorFilter, options: actorOptions, onChange: (v) => setActorFilter(v), searchable: true },
              ]}
            />
            ...
            {selectedRuns.size > 0 && (
              <button
                onClick={selectedRuns.size >= 2 ? () => setShowCompare(true) : undefined}
                className={cn(
              ...
              >
                ...
                <span
                  role="button"
                  onClick={(e) => { e.stopPropagation(); setSelectedRuns(new Set()); }}
          ...
          <div className="mt-4 mb-2 flex items-center justify-between">
            <span className="text-[11px] font-mono text-muted-foreground/40">
              ...
              {hasActiveFilters ? ` · ${filteredRuns.length} shown on this page` : ""}
              {totalPages > 1 && ` · page ${page} of ${totalPages}`}
            </span>
            {loading && <Loader2 className="w-3.5 h-3.5 text-muted-foreground/40 animate-spin" />}
          </div>
        </>
      )}
      </div>
      ...
      {showCompare && selectedRuns.size >= 2 ? (
        ...
        <>
          {/* Runs list */}
          <div className={cn("border border-border/60 divide-y divide-border/40 transition-opacity duration-200", loading && "opacity-40 pointer-events-none")}>
                ...
                >
                  ...
                  >
                    ...
                    <div className="shrink-0 text-right hidden sm:block">
                      ...
                      {duration && (
                        <div className="flex items-center gap-1 text-[11px] text-muted-foreground/30 justify-end mt-0.5" suppressHydrationWarning>
