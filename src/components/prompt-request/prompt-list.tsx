...
export function PromptList({ owner, repo, promptRequests }: PromptListProps) {
  ...
  const filtered = useMemo(() => {
    let list = promptRequests.filter((pr) => {
      if (tab === "open") return pr.status === "open" || pr.status === "processing";
  ...
  const tabItems: { key: StatusTab; label: string; icon: React.ReactNode; count: number }[] = [
    { key: "open", label: "Open", icon: <CircleDot className="w-3 h-3" />, count: counts.open },
    { key: "completed", label: "Completed", icon: <CheckCircle2 className="w-3 h-3" />, count: counts.completed },
    { key: "rejected", label: "Closed", icon: <XCircle className="w-3 h-3" />, count: counts.rejected },
  ];
        ...
        <div className="divide-y divide-border mx-4">
          {filtered.map((pr) => (
            ...
            >
              ...
              {(pr.status === "open" || pr.status === "processing") && (
                <button
                  onClick={(e) => handleClose(e, pr.id)}
