  ...
  async function handleImportFile(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    try {
      ...
      const res = await fetch("/api/code-themes", {
        ...
        body: JSON.stringify({ name: json.name || file.name.replace(/\.json$/, ""), themeJson: json }),
      });
      if (!res.ok) {
    ...
    <div className="divide-y divide-border overflow-y-auto max-h-[calc(100dvh-220px)]">
      ...
      <div className="px-4 py-4">
        ...
        <ThemeGrid
          ...
          onDelete={(id) => customDark.some((c) => c.id === id) && handleDeleteCustom(id)}
      ...
      <div className="px-4 py-4">
        ...
        <ThemeGrid
          ...
          onDelete={(id) => customLight.some((c) => c.id === id) && handleDeleteCustom(id)}
      ...
      <div className="px-4 py-4">
        ...
        {importError && (
          <p className="text-[11px] text-destructive mt-2">{importError}</p>
        )}
      </div>
      ...
      <div className="px-4 py-4">
        ...
        <div
          ...
          style={{
            backgroundColor: "var(--code-theme-bg, var(--code-bg))",
          }}
        >
          <pre
      ...
      {themes.map((theme) => {
        ...
        return (
          ...
          >
            ...
            {/* Actions */}
            <div className="flex items-center gap-1 shrink-0">
              {isActive && <Check className="size-3.5 text-success" />}
              {isCustom && (
                <button
                  onClick={(e) => {
                    e.stopPropagation();
