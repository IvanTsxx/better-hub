...
function getEventDescription(event: RepoEvent, base: string): { verb: string; detail: string; href: string | null } {
  ...
  switch (event.type) {
    case "PushEvent": {
      ...
      const commitHref = commitCount === 1 && firstCommit?.sha
    ...
    case "CreateEvent":
      return {
        ...
        detail: "",
        href: p?.ref_type === "branch" && p?.ref ? `${base}/tree/${p.ref}` : null,
      };
    case "DeleteEvent":
      return { verb: `deleted ${p?.ref_type ?? "branch"} ${p?.ref ?? ""}`, detail: "", href: null };
    case "ReleaseEvent":
      return {
...
export function RepoActivityView({ owner, repo, events, commitActivity }: RepoActivityViewProps) {
  ...
  const filtered = filter === "all"
    ...
    : filter === "push"
      ...
      : filter === "pr"
        ? significant.filter((e) => ["PullRequestEvent", "PullRequestReviewEvent", "PullRequestReviewCommentEvent"].includes(e.type))
        : significant.filter((e) => ["IssuesEvent", "IssueCommentEvent"].includes(e.type));
  ...
  return (
    <div className="space-y-6">
      ...
      {weeks.length > 0 && (
        <div className="border border-border/40 rounded-lg p-4">
          <div className="flex items-baseline gap-2 mb-4">
            <h3 className="text-sm font-medium text-foreground">Commit Activity</h3>
            <span className="text-xs font-mono text-muted-foreground/60">{formatNumber(total)} commits in 24 weeks</span>
          </div>
                ...
                >
                  ...
                  {hoveredWeek === i && (
                    <div className="absolute bottom-full mb-2 left-1/2 -translate-x-1/2 z-10 whitespace-nowrap px-2 py-1 text-[9px] font-mono bg-card text-foreground rounded-md shadow-lg border border-border/60">
                      {weekDate.toLocaleDateString("en-US", { month: "short", day: "numeric" })} — {week.total} commit{week.total !== 1 ? "s" : ""}
                    </div>
                  )}
                </div>
              );
      ...
      <div className="flex items-center gap-1 text-[11px] font-mono">
        ...
        <span className="text-muted-foreground/40 ml-2">{filtered.length} events</span>
      </div>
      ...
      <div className="space-y-1">
        {filtered.length === 0 ? (
          ...
          filtered.map((event, i) => {
            const { verb, detail, href } = getEventDescription(event, base);
            const inner = (
              <div className="flex items-start gap-3 py-2.5 px-3 rounded-md transition-colors hover:bg-muted/30 group border border-transparent hover:border-border/30">
                ...
                <div className="min-w-0 flex-1">
                  ...
                  {detail && (
                    <p className="text-xs font-mono text-muted-foreground/50 truncate mt-1">{detail}</p>
                  )}
                </div>
