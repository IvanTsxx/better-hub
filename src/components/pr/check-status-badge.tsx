    ...
    <div>
      ...
      >
        <ChevronRight className={cn(
          "w-2.5 h-2.5 text-muted-foreground/30 transition-transform duration-150 shrink-0",
          expanded && "rotate-90"
        )} />
        <span className="text-muted-foreground/50 shrink-0">{group.provider.icon}</span>
        <span className="font-mono text-[11px] font-medium text-foreground/80 flex-1 text-left truncate">
          {group.provider.name}
        </span>
        ...
        {group.failed > 0 && (
          <span className="font-mono text-[9px] text-destructive/70 tabular-nums shrink-0">
            {group.failed} failed
          </span>
        )}
        {group.failed === 0 && group.pending > 0 && (
          <span className="font-mono text-[9px] text-warning/70 tabular-nums shrink-0">
            {group.pending} pending
          </span>
        )}
        {group.failed === 0 && group.pending === 0 && (
          <CheckCircle2 className="w-3 h-3 text-success/50 shrink-0" />
        )}
      </button>
      ...
      {expanded && (
        <div className="border-t border-border/30">
          {group.checks.map((check, i) => (
            ...
            >
              <CheckIcon state={check.state} className="w-2.5 h-2.5 shrink-0" />
              <span className="font-mono text-[10px] truncate flex-1 text-foreground/70">
                {check.name}
              </span>
...
export function CheckStatusBadge({
  ...
  const grouped = useMemo(() => groupChecksByProvider(checkStatus.checks), [checkStatus.checks]);
