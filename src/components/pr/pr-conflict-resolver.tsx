...
type HunkResolutionStatus = "pending" | "accepted-base" | "accepted-head" | "accepted-both" | "custom";
...
export function PRConflictResolver({
          ...
          if (file.autoResolved) {
            map.set(file.path, {
              ...
              hunkResolutions: file.hunks.map((h) => ({
                status: h.type === "clean" ? "accepted-base" : "pending",
                resolvedLines: h.resolvedLines || [],
              })),
            });
          } else {
            if (!firstConflictFile) firstConflictFile = file.path;
            map.set(file.path, {
              status: "pending",
              hunkResolutions: file.hunks.map((h) => ({
                status: h.type === "clean" ? "accepted-base" : "pending",
                resolvedLines: h.type === "clean" ? h.resolvedLines || [] : [],
              })),
            });
          }
        }
  ...
  const updateHunkResolution = useCallback(
    (filePath: string, hunkIdx: number, status: HunkResolutionStatus, lines: string[]) => {
      setResolutions((prev) => {
  ...
  const acceptBoth = useCallback(
    (filePath: string, hunkIdx: number, baseLines: string[], headLines: string[]) => {
      updateHunkResolution(filePath, hunkIdx, "accepted-both", [...baseLines, ...headLines]);
    },
    [updateHunkResolution]
  );
  ...
  useEffect(() => {
    const handler = (e: KeyboardEvent) => {
      ...
      if (e.key === "j") {
        ...
        if (hunk.type === "conflict") acceptBase(activeFile, activeHunkIdx, hunk.baseLines || []);
        e.preventDefault();
      } else if (e.key === "2" && conflictHunkIndices.includes(activeHunkIdx)) {
        const hunk = activeFileData.hunks[activeHunkIdx];
        if (hunk.type === "conflict") acceptHead(activeFile, activeHunkIdx, hunk.headLines || []);
        e.preventDefault();
      } else if (e.key === "3" && conflictHunkIndices.includes(activeHunkIdx)) {
        const hunk = activeFileData.hunks[activeHunkIdx];
        if (hunk.type === "conflict") acceptBoth(activeFile, activeHunkIdx, hunk.baseLines || [], hunk.headLines || []);
        e.preventDefault();
  ...
  }, [activeFile, activeFileData, activeFileRes, activeHunkIdx, conflictHunkIndices, conflictFiles, allResolved, acceptBase, acceptHead, acceptBoth, router]);
    ...
    return (
      <div className="flex-1 flex items-center justify-center">
        <div className="flex items-center gap-2 text-muted-foreground">
          ...
          <span className="text-xs font-mono">Analyzing conflicts...</span>
        </div>
      </div>
    );
  }
    ...
    return (
      <div className="flex-1 flex items-center justify-center">
        <div className="text-center space-y-2">
          ...
          <p className="text-xs font-mono text-muted-foreground">No conflicts found</p>
          <button
            onClick={() => router.back()}
            className="text-[11px] font-mono text-foreground/70 hover:text-foreground underline cursor-pointer"
          >
    ...
    <div ref={mainRef} className="flex-1 flex flex-col min-h-0" tabIndex={-1}>
      ...
      {showShortcuts && (
        <div className="shrink-0 px-4 py-2 relative after:absolute after:bottom-0 after:left-[5%] after:right-[5%] after:h-px after:bg-gradient-to-r after:from-transparent after:via-border after:to-transparent bg-muted/20 text-[10px] font-mono text-muted-foreground flex flex-wrap gap-x-4 gap-y-1">
          <span><kbd className="px-1 py-0.5 bg-muted border border-border rounded text-[9px]">j</kbd>/<kbd className="px-1 py-0.5 bg-muted border border-border rounded text-[9px]">k</kbd> prev/next hunk</span>
          <span><kbd className="px-1 py-0.5 bg-muted border border-border rounded text-[9px]">[</kbd>/<kbd className="px-1 py-0.5 bg-muted border border-border rounded text-[9px]">]</kbd> prev/next file</span>
          <span><kbd className="px-1 py-0.5 bg-muted border border-border rounded text-[9px]">1</kbd> accept base</span>
          <span><kbd className="px-1 py-0.5 bg-muted border border-border rounded text-[9px]">2</kbd> accept head</span>
          <span><kbd className="px-1 py-0.5 bg-muted border border-border rounded text-[9px]">3</kbd> accept both</span>
          <span><kbd className="px-1 py-0.5 bg-muted border border-border rounded text-[9px]">⌘↵</kbd> commit</span>
          <span><kbd className="px-1 py-0.5 bg-muted border border-border rounded text-[9px]">Esc</kbd> exit</span>
        </div>
      )}
          ...
          <div className="px-3 py-2 relative after:absolute after:bottom-0 after:left-[10%] after:right-[10%] after:h-px after:bg-gradient-to-r after:from-transparent after:via-border/50 after:to-transparent">
            <div className="h-1.5 bg-muted rounded-full overflow-hidden">
              <div
                ...
                style={{ width: totalConflictFiles > 0 ? `${(resolvedCount / totalConflictFiles) * 100}%` : "0%" }}
              />
            </div>
          </div>
          ...
          {conflictFiles.length > 0 && (
            <div className="py-1">
              ...
              {conflictFiles.map((file) => {
                const res = resolutions.get(file.path);
                ...
                return (
                  ...
                  >
                    ...
                    <span className="text-[11px] font-mono truncate">{file.path.split("/").pop()}</span>
                  </button>
                );
              })}
            </div>
            ...
            <>
              ...
              <div className="shrink-0 sticky top-0 z-10 flex items-center justify-between px-4 py-2 relative after:absolute after:bottom-0 after:left-[3%] after:right-[3%] after:h-px after:bg-gradient-to-r after:from-transparent after:via-border after:to-transparent bg-background/95 backdrop-blur-sm">
                <div className="flex items-center gap-2 min-w-0">
                  ...
                  <span className="text-xs font-mono truncate">{activeFileData.path}</span>
                  {activeFileData.hasConflicts && (
                    <span className="text-[10px] font-mono text-amber-500">
                      {conflictHunkIndices.filter(
                        (i) => activeFileRes.hunkResolutions[i]?.status === "pending"
                      ).length}{" "}
                      conflict{conflictHunkIndices.filter(
                        (i) => activeFileRes.hunkResolutions[i]?.status === "pending"
                      ).length !== 1 ? "s" : ""} remaining
                    </span>
                  )}
                </div>
                {activeFileData.hasConflicts && activeFileRes.status !== "auto-resolved" && (
                  <div className="flex items-center gap-1">
                    <button
                      onClick={() => acceptAllBase(activeFile!)}
                    ...
                    <button
                      onClick={() => acceptAllHead(activeFile!)}
                      className="px-2 py-1 text-[10px] font-mono rounded-sm bg-muted/50 dark:bg-white/[0.04] text-muted-foreground hover:text-foreground hover:bg-muted dark:hover:bg-white/[0.08] transition-colors cursor-pointer"
                    >
                      Accept all head
                    </button>
              ...
              <div className="flex-1 p-4 space-y-3">
                {activeFileData.hunks.map((hunk, idx) => (
                  <HunkView
                    key={idx}
                    hunk={hunk}
                    hunkIdx={idx}
                    filePath={activeFileData.path}
                    resolution={activeFileRes.hunkResolutions[idx]}
                    isActive={activeHunkIdx === idx}
                    onFocus={() => setActiveHunkIdx(idx)}
                    onAcceptBase={(lines) => acceptBase(activeFileData.path, idx, lines)}
                    onAcceptHead={(lines) => acceptHead(activeFileData.path, idx, lines)}
                    onAcceptBoth={(base, head) => acceptBoth(activeFileData.path, idx, base, head)}
                    onCustom={(lines) => updateHunkResolution(activeFileData.path, idx, "custom", lines)}
        ...
        <div className="flex items-center gap-3">
            ...
            {allResolved ? (
              <div className="flex items-center gap-3">
                <span className="shrink-0 text-[10px] font-mono text-green-600 dark:text-green-400 flex items-center gap-1">
                  ...
                  All {totalConflictFiles} file{totalConflictFiles !== 1 ? "s" : ""} resolved
                </span>
                <input
                  type="text"
                  value={commitMessage}
              ...
              <span className="text-[10px] font-mono text-muted-foreground">
                Resolve all conflict files to commit ({resolvedCount}/{totalConflictFiles})
              </span>
            )}
          </div>
          ...
          {commitResult && (
            <span
              className={cn(
                "text-[10px] font-mono",
                commitResult.type === "error" ? "text-destructive" : "text-green-600 dark:text-green-400"
              )}
            >
          ...
          <button
            ...
            disabled={!allResolved || isPending || !commitMessage.trim()}
            className={cn(
      ...
      <div className="group">
        ...
        {!collapsed && (
          <pre className="text-[11px] font-mono leading-relaxed text-muted-foreground/40 overflow-x-auto max-h-40 overflow-y-auto bg-muted/20 px-3 py-1.5 border border-border/30 rounded-sm">
            {isLong ? (
              <>
                {lines.slice(0, MAX_PREVIEW).map((l, i) => (
                  <div key={i}>{l || " "}</div>
                ))}
                <div className="text-muted-foreground/20 my-0.5">··· {lines.length - MAX_PREVIEW * 2} lines ···</div>
                {lines.slice(-MAX_PREVIEW).map((l, i) => (
                  <div key={`end-${i}`}>{l || " "}</div>
                ))}
              </>
            ) : (
  ...
  return (
    <div
      ...
      className={cn(
        ...
        isActive ? "border-blue-500/40 ring-1 ring-blue-500/20" : "border-border",
        ...
        <div className="grid grid-cols-2 relative after:absolute after:top-[10%] after:bottom-[10%] after:left-1/2 after:w-px after:bg-gradient-to-b after:from-transparent after:via-border/50 after:to-transparent">
          ...
          <div className="min-w-0">
            ...
            <pre className="text-[11px] font-mono leading-relaxed overflow-x-auto px-3 py-1.5 bg-red-500/[0.03] min-h-[2rem]">
              {baseLines.length > 0 ? (
                baseLines.map((l, i) => (
                  <div key={i} className="text-red-700/70 dark:text-red-400/60">
                    {l || " "}
                  </div>
                ))
              ) : (
                <div className="text-muted-foreground/30 italic">empty</div>
              )}
            </pre>
          </div>
          ...
          <div className="min-w-0">
            ...
            <pre className="text-[11px] font-mono leading-relaxed overflow-x-auto px-3 py-1.5 bg-green-500/[0.03] min-h-[2rem]">
              {headLines.length > 0 ? (
                headLines.map((l, i) => (
                  <div key={i} className="text-green-700/70 dark:text-green-400/60">
                    {l || " "}
                  </div>
                ))
              ) : (
        ...
        <div className="relative before:absolute before:top-0 before:left-[5%] before:right-[5%] before:h-px before:bg-gradient-to-r before:from-transparent before:via-border/30 before:to-transparent">
          ...
          <pre className="text-[11px] font-mono leading-relaxed overflow-x-auto px-3 py-1.5 bg-blue-500/[0.02] max-h-40 overflow-y-auto">
            {resolution.resolvedLines.length > 0 ? (
              ...
              <div className="text-muted-foreground/30 italic">empty (lines removed)</div>
            )}
          </pre>
        </div>
      )}
