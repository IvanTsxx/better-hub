...
function diff(from: string[], to: string[]): DiffOp[] {
  ...
  const rawOps: Array<{ type: "equal" | "insert" | "delete"; fromIdx: number; toIdx: number; line: string }> = [];

  while (i > 0 || j > 0) {
    if (i > 0 && j > 0 && from[i - 1] === to[j - 1]) {
      rawOps.push({ type: "equal", fromIdx: i - 1, toIdx: j - 1, line: from[i - 1] });
      i--;
      j--;
      ...
      rawOps.push({ type: "delete", fromIdx: i - 1, toIdx: j, line: from[i - 1] });
      i--;
    }
  }
  ...
  for (const op of ops) {
    if (op.type === "equal") {
      ...
      if (last && last.ancestorEnd === start) {
        ...
        regions.push({ ancestorStart: start, ancestorEnd: ancestorIdx, changedLines: [] });
      }
    } else {
      // insert
      const last = regions[regions.length - 1];
      if (last && last.ancestorEnd === ancestorIdx) {
        ...
        regions.push({ ancestorStart: ancestorIdx, ancestorEnd: ancestorIdx, changedLines: [...op.lines] });
      }
    }
  }
...
function extractEditsFromOps(ops: DiffOp[]): EditRegion[] {
  ...
  for (const op of ops) {
    if (op.type === "equal") {
      ...
      if (prev && prev.ancestorEnd === ancestorIdx && prev.changedLines.length === 0) {
    ...
    if (be && he) {
      ...
      if (be.ancestorEnd <= heStart) {
        ...
        if (ancestorPos < beStart) {
          hunks.push({ type: "clean", resolvedLines: ancestor.slice(ancestorPos, beStart) });
        }
        ...
        if (ancestorPos < heStart) {
          hunks.push({ type: "clean", resolvedLines: ancestor.slice(ancestorPos, heStart) });
        }
        ...
        const sameChange =
          ...
          be.changedLines.every((l, idx) => l === he.changedLines[idx]);
        ...
        if (sameChange) {
          ...
          if (ancestorPos < overlapStart) {
            hunks.push({ type: "clean", resolvedLines: ancestor.slice(ancestorPos, overlapStart) });
          }

          hunks.push({
            type: "conflict",
            ancestorLines: ancestor.slice(overlapStart, overlapEnd),
          ...
          while (bi < baseEdits.length && baseEdits[bi].ancestorStart < overlapEnd) bi++;
          while (hi < headEdits.length && headEdits[hi].ancestorStart < overlapEnd) hi++;
        }
      }
    } else if (be) {
      // Only base edits remaining
      if (ancestorPos < be.ancestorStart) {
        hunks.push({ type: "clean", resolvedLines: ancestor.slice(ancestorPos, be.ancestorStart) });
      }
      hunks.push({ type: "clean", resolvedLines: be.changedLines });
      ancestorPos = be.ancestorEnd;
      bi++;
      ...
      if (ancestorPos < he.ancestorStart) {
        hunks.push({ type: "clean", resolvedLines: ancestor.slice(ancestorPos, he.ancestorStart) });
      }
      hunks.push({ type: "clean", resolvedLines: he.changedLines });
      ancestorPos = he.ancestorEnd;
      hi++;
