...
const CHAT_SERVICES = [
	...
	},
	{
		...
		label: "Copilot",
		icon: (
			<svg viewBox="0 0 1322.9 1147.5" className="w-4 h-4" fill="currentColor" xmlns="http://www.w3.org/2000/svg">
	...
	},
	{
		...
		label: "Cursor",
		icon: (
			<svg viewBox="0 0 466.73 532.09" className="w-4 h-4" fill="currentColor" xmlns="http://www.w3.org/2000/svg">
				...
				{dropdownOpen && (
					<div className="absolute right-0 top-full mt-1.5 w-48 rounded-lg border border-border bg-popover shadow-lg z-50 py-1 animate-in fade-in-0 zoom-in-95 duration-100">
						{CHAT_SERVICES.map((service) => (
							<button
								key={service.id}
								onClick={() => handleOpenIn(service.createUrl)}
							...
							>
								<span className="shrink-0 text-muted-foreground/50">{service.icon}</span>
